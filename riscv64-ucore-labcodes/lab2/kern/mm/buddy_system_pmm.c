...
// //     struct Page *relative_block_addr = block_addr - mem_begin;
...
//     unsigned int buddy_ppn = first_ppn + ((1 << order) ^ (page2ppn(page) - first_ppn)); // first_ppn是在ppm.h中新声明的全局变量，表示第一个可分配物理内存页的下标
//     // cprintf("[!]BS: Page NO.%d 's buddy page on order %d is: %d\n", page2ppn(page), order, buddy_ppn);
//     cprintf("page的ppn为%d", page2ppn(page));
//     cprintf("buddy的ppn为%d", buddy_ppn);
//     if (buddy_ppn > page2ppn(page)) // 伙伴块是右块
//     {
